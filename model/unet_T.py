...
class U_Net_top2bottom2(nn.Module):
    ...
    def forward(self, x):
        ...
        d2 = torch.cat((e1sam,e1, d2), dim=1)  # 将e1特征图与d1特征图横向拼接
        d2 = self.Up_conv2(d2)
...
class U_Net_aspp(nn.Module):
    ...
    def __init__(self, out_ch=2):
        ...
        self.aspp = ASPP(filters[4],filters[3])
...
class U_Net_abam(nn.Module):

    # 输入是3个通道的RGB图，输出是0或1——因为我的任务是2分类任务
    def __init__(self, out_ch=2):
        super(U_Net_abam, self).__init__()
...
if __name__ =="__main__":
    ...
    ##     U_Net_o, U_Net_dws, U_Net_dws2, U_Net_top2bottom ,U_Net_top2bottom
    net = U_Net_top2bottom2(2)
