...
class U_Net_Multi_add(nn.Module):
    '''
    unet模型 + 多尺度 + 单层卷积     U_Net_Multi
    num_classes 类别数
    '''
    def __init__(self, num_classes=2):
        super(U_Net_Multi_add, self).__init__()
        ...
        self.Up5 = up_conv(filters[4], filters[3])
        self.Up_conv5 = conv_block(int(filters[4]), filters[3])

        self.Up4 = up_conv(filters[3], filters[2])
        self.Up_conv4 = conv_block(int(filters[3]), filters[2])

        self.Up3 = up_conv(filters[2], filters[1])
        self.Up_conv3 = conv_block(int(filters[2]), filters[1])
